...
> **Council-model notice:** the ward probability, coattail, and ward-poll
> conversion sections in this historical v0.2 document are superseded by
> [Toronto Council Race Model v1](council-model-spec-v1.md). Council snapshot
> schema v3 publishes evidence-based race assessments and withholds candidate
> odds until the documented calibration gates pass.
...
$$
\mu_j = \mu_{\text{tier}(j)} + w_f \cdot \log(F_j) + w_a \cdot \left(L_w^{m(j)} + M_{m(j)}^{\text{draw}} - \bar{M}_{m(j)}\right) + \text{split}_j
$$
...
- $M_{m(j)}^{\text{draw}} - \bar{M}_{m(j)}$ is the aligned mayoral candidate's deviation in this simulation draw from their current polling average. Centring on the observed average avoids an arbitrary city-wide neutral point while preserving correlated mayoral/council movement.
- A newly entered mayoral candidate with no historical ward result receives a neutral geographic lean ($L_w^{m(j)}=0$) until ward evidence exists. Their aligned council candidates can still be helped or hurt when the mayoral draw moves above or below the candidate's polling average. If the alignment is absent from the current mayoral scenario, no alignment boost is applied.
...
1. **Draw a mayoral outcome** from the polling distribution (see Part 8). The aggregated mayoral vote shares are used as the mean of a Dirichlet distribution, which ensures the drawn vote shares for all candidates sum to 100%. The concentration of the Dirichlet (controlling how much variance there is around the polling average) is derived from the polling average's effective sample size, which increases as more polls are aggregated. This draw includes all tracked mayoral candidates.
   ...
   The effective sample size is the recency-decay-weighted sum of the aggregated polls' sample sizes (same half-life as the aggregator), so uncertainty widens when polling is sparse or stale. On top of sampling error, a **campaign-drift term** is added: vote intentions move between now and election day, modelled as a random walk contributing an editorial 1.5 points of share standard deviation per square-root month remaining (≈3 points four months out, zero on election day). The Dirichlet concentration is set so the front-runner's share variance equals sampling variance plus drift variance. Both components, and the resulting per-candidate mayoral win probabilities, are published in the model snapshot (`mayoral_uncertainty`, `mayoral_win_probabilities`).
...
For running incumbents, the published probability interval is the 10th–90th percentile of the per-draw modelled win probabilities. It reflects uncertainty generated by mayoral mood and modelled race noise under the fixed editorial parameters; it is not a Monte Carlo standard-error interval. Open seats have no incumbent interval. Conditional council-composition summaries are published only when at least one simulation draw has the named mayoral winner; otherwise their mean and standard deviation are explicitly unavailable rather than reported as zero.
