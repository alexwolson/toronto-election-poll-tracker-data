...
Reading the ladder is essential: **a lower rung failing to beat a simpler one is the ladder working (ADR 0030: "the simpler specification remains authoritative"), not a defect.** As of the six-cycle corpus the bridge's *relative* comparison has started to pass on regular elections, but the endpoint is still held `not_qualified` by other ADR gates (an unconfigured reliability check; the 2023 by-election dragging the all-elections aggregate). The authoritative reading is unchanged — the comparator stays authoritative until the endpoint clears **every** gate — see below.
...
| Qualification ladder (comparator vs bridge) | Built; **result: not_qualified** (but relative criteria now pass on regular elections; held by the unconfigured reliability gate + the 2023 all-elections drag) | `mayoral_endpoint_qualification.py`, `scripts/evaluate_mayoral_endpoint.py` |
...
**Qualification status: `not_qualified`** (re-run 2026-08-19 on the six-cycle 2006–2023 corpus) — **but the relative comparison now passes on regular elections for the first time.** On **regular elections (5 cycles: 2006/2010/2014/2018/2022)** the firm-balanced bridge now beats the latest-sample comparator on *both* ADR 0030 relative criteria: majority-of-cycles (**3/5 improve**) and aggregate winning-margin CRPS (**0.0397 vs 0.0446**), with log-loss not worse — so `relative_qualifies == True` for that population. It is still held `not_qualified` by two separate gates, neither of which should be "fixed" to force a pass:
> 1. **The endpoint reliability check is unconfigured** (`endpoint_reliability.configured == False` → `passed == False`). This is an ADR-defined gate (calibration/coverage reliability), not a metric to tune. Whether and how to configure it is a **methodology decision for a human**, not something to hack so the endpoint qualifies.
> 2. **On all 6 elections the bridge is worse in aggregate** (0.0495 vs 0.0467), still dragged by the 2023 by-election outlier (bridge CRPS 0.091), even though it improves the majority of cycles there too (4/6). The by-election remains the single cycle that can swing the all-elections aggregate.

Per-cycle winning-margin CRPS (all-elections fit): bridge beats comparator on **2006 (0.054 vs 0.067), 2010 (0.026 vs 0.029), 2018 (0.048 vs 0.063), 2022 (0.041 vs 0.044)**; comparator wins **2014 (0.028 vs 0.037)** and the **2023 by-election (0.049 vs 0.091)**. This is still an **n=5/6 result** — a single cycle can swing it, and the regular-elections pass rests on a 3/5 majority. Per ADR 0030 the comparator stays authoritative until the endpoint qualifies through *all* gates. **Report this; do not tune to it.** The live levers remain more whole cycles (2000/2003) and the deliberate, ADR-level decision on the reliability check — not adjusting the model to the scoreboard.
