...
**Qualification status: `not_qualified`** (re-run 2026-08-19 on the seven-cycle 2003–2023 corpus) — **but the relative comparison passes on the authoritative population, more robustly than before, and only the reliability gate remains.** Per **ADR 0040**, by-elections are excluded from the qualification gate: the **regular-elections population (6 cycles: 2003/2006/2010/2014/2018/2022) is authoritative**, and the all-elections decision (which includes the structurally-different 2023 by-election) is retained and reported but **report-only**. On the regular population the firm-balanced bridge beats the latest-sample comparator on *both* ADR 0030 relative criteria: majority-of-cycles (**4/6 improve**) and aggregate winning-margin CRPS (**0.0378 vs 0.0397**), log-loss not worse — so `relative_qualifies == True`. The endpoint is nonetheless held `not_qualified` by a single remaining gate, which must **not** be "fixed" to force a pass:
...
Per-cycle winning-margin CRPS (all-elections fit): bridge beats comparator on **2003 (0.017 vs 0.022), 2006 (0.061 vs 0.067), 2010 (0.028 vs 0.030), 2018 (0.051 vs 0.061)**; comparator wins **2014 (0.042 vs 0.031), 2022 (0.046 vs 0.043)** and the report-only **2023 by-election (0.067 vs 0.040)**. Adding the competitive 2003 race strengthened the regular-elections pass (from a 3/5 to a 4/6 majority, with a wider aggregate margin). This is still an **n=6/7 result** a single cycle could move. Per ADR 0030 the comparator stays authoritative until the endpoint clears *every* gate. **Report this; do not tune to it.** The one live lever is the deliberate, ADR-level decision on the reliability check — not adjusting the model to the scoreboard.
