...
Reading the ladder is essential: **a lower rung failing to beat a simpler one is the ladder working (ADR 0030: "the simpler specification remains authoritative"), not a defect.** As of the six-cycle corpus the bridge's *relative* comparison passes on the authoritative regular-elections population (ADR 0040 excludes the structurally-different 2023 by-election from the gate, keeping it report-only), but the endpoint is still held `not_qualified` by the deferred absolute reliability gate. The authoritative reading is unchanged — the comparator stays authoritative until the endpoint clears **every** gate — see below.
...
| Historical corpus + reconstruction | Built; **2006–2023 complete** (proxies resolved, blocker cleared; 2006 + 2010 added 2026-08-19); **2000/2003 pending** | `historical_mayoral.py`, `scripts/reconstruct_historical_mayoral.py` |
...
| Qualification ladder (comparator vs bridge) | Built; **result: not_qualified** (relative criteria now pass on the authoritative regular-elections population per ADR 0040; held solely by the deferred reliability gate; by-election report-only) | `mayoral_endpoint_qualification.py`, `scripts/evaluate_mayoral_endpoint.py` |
...
The corpus holds **six reconstructed cycles (2006, 2010, 2014, 2018, 2022, 2023), 109 samples / 247 readings**. The 2014–2023 legacy-proxy backlog is **fully resolved** (2026-08-19): every proxy is `mapped` (143) or `no_public_source` (9, documented-unavailable), zero `unresolved`, and the `unresolved_legacy_poll_samples` calibration blocker is **cleared** (`blocker_codes == ()`). Sources span Forum, Mainstreet, Nanos, Ipsos, Angus Reid, Pollara, and EKOS first-party reports plus, where no first-party PDF survives, newspaper/CP24 toplines (`document_role` article). What remains for M1 is the **older cycles 2000 and 2003** (not the 2006–2023 corpus). Restricted Forum/Borealis material is optional. See [unresolved-proxy-source-hunt.md](research/unresolved-proxy-source-hunt.md) and the `historical-mayoral-*` notes.
...
**Qualification status: `not_qualified`** (re-run 2026-08-19 on the six-cycle 2006–2023 corpus) — **but the relative comparison now passes on the authoritative population, and only the reliability gate remains.** Per **ADR 0040**, by-elections are excluded from the qualification gate: the **regular-elections population (5 cycles: 2006/2010/2014/2018/2022) is authoritative**, and the all-elections decision (which includes the structurally-different 2023 by-election) is retained and reported but **report-only**. On the regular population the firm-balanced bridge now beats the latest-sample comparator on *both* ADR 0030 relative criteria: majority-of-cycles (**3/5 improve**) and aggregate winning-margin CRPS (**0.0397 vs 0.0446**), log-loss not worse — so `relative_qualifies == True`. The endpoint is nonetheless held `not_qualified` by a single remaining gate, which must **not** be "fixed" to force a pass:
> **The endpoint reliability check is fail-closed** (`MAYORAL_ENDPOINT_ABSOLUTE_MAXIMUM_SCORES is None` → `endpoint_reliability.configured == False` → `passed == False`). This is an ADR-defined absolute gate (calibration/coverage reliability), deliberately deferred. Whether and how to configure it is a **methodology decision for a human**, not something to hack so the endpoint qualifies.

Per-cycle winning-margin CRPS (all-elections fit): bridge beats comparator on **2006 (0.054 vs 0.067), 2010 (0.026 vs 0.029), 2018 (0.048 vs 0.063), 2022 (0.041 vs 0.044)**; comparator wins **2014 (0.028 vs 0.037)** and the report-only **2023 by-election (0.049 vs 0.091)**. This is still an **n=5/6 result** — the regular-elections pass rests on a 3/5 majority a single cycle could flip. Per ADR 0030 the comparator stays authoritative until the endpoint clears *every* gate. **Report this; do not tune to it.** The live levers are the deliberate, ADR-level decision on the reliability check and more whole cycles (2000/2003) — not adjusting the model to the scoreboard.
...
- **M1 — Finish the historical poll corpus** (target cycles 2000–2023, 1997 excluded). **2006–2023 done** (2026-08-19): 2014–2023 backlog resolved and blocker cleared; 2010 and 2006 built fresh (Final-Ballot-window polls only; pre-nomination polls intentionally out-of-window; qualification re-run — the bridge's relative criteria now pass on regular elections). **Remaining: older cycles 2000 (results-only) and 2003.**
...
- **Record findings.** Investigations become dated research notes in `docs/research/`; decisions become terse, declarative ADRs in `docs/adr/` (next number **0041**). Cross-link them.
...
- Decisions — [docs/adr/](adr/) 0001–0040
