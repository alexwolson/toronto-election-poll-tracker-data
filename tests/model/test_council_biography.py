...
def test_multi_term_councillor_record_spans_the_boundary_change() -> None:
    ...
    bio = build_candidate_biography("filion john", _results())
...
def test_returning_former_councillor_record() -> None:
    ...
    bio = build_candidate_biography("layton mike", _results())
...
def test_identity_is_exact_key_and_does_not_conflate_by_surname() -> None:
    # 'clayton jones' contains the 'layton' substring but is a different person;
    # exact-key identity must never pull that 2014 ward-39 row into Layton.
    bio = build_candidate_biography("layton mike", _results())
...
def test_single_term_incumbent_record() -> None:
    bio = build_candidate_biography("dianne saxe", _results())
...
def test_build_all_biographies_covers_every_distinct_candidate() -> None:
    ...
    assert "filion john" in bios
    assert "layton mike" in bios
    assert len(bios) == len({r.candidate_key for r in results})
    # a built biography contains exactly that key's appearances
    assert bios["filion john"].council_wins == 5
