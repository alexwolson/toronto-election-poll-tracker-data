...
def test_only_audited_poll_sources_enter_the_canonical_seam() -> None:
    ...
    assert len(corpus.poll_samples) == 96
    assert len(corpus.poll_readings) == 234
    assert len(corpus.poll_responses) == 1368
    assert len(corpus.source_documents) == 118
    assert len(corpus.poll_sample_documents) == 120
    assert all(
    ...
    assert sum(value == "general_vote_intention" for value in purposes.values()) == 231
