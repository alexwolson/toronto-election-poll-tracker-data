...
def test_only_audited_poll_sources_enter_the_canonical_seam() -> None:
    ...
    assert len(corpus.poll_samples) == 67
    assert len(corpus.poll_readings) == 149
    assert len(corpus.poll_responses) == 991
    assert len(corpus.source_documents) == 89
    assert len(corpus.poll_sample_documents) == 90
    assert all(
    ...
    assert sum(value == "general_vote_intention" for value in purposes.values()) == 146
...
def test_audit_counts_inventory_without_calling_it_calibration_ready() -> None:
    ...
    assert audit.source_verified_sample_count == 67
    assert audit.source_verified_reading_count == 149
